...
function castChosenSpell(g: Game, m: Monster): boolean {
  ...
  switch (choice) {
    ...
    case 'DRAIN_MANA': { if (p.csp > 0) { const d = Math.min(p.csp, randint1(level) + 1); p.csp -= d; m.hp = Math.min(m.maxhp, m.hp + d * 6); g.msg.add(seen ? `${name} draws psychic energy from you!` : 'Something drains your mana!', '#ffb0b0'); } else monsterLearn(g, 'NO_MANA'); return true; }
